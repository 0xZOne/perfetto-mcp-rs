...
MCP 的客户端）里打开一个 Perfetto trace 文件（`.pftrace` / `.perfetto-trace` / `.bin` 等都行——按内容嗅探），直接用
PerfettoSQL 查询分析。
...
| 工具 | 用途 |
|---|---|
| `load_trace` | 打开 Perfetto trace 文件（`.pftrace` / `.perfetto-trace` / `.bin` 等任何 trace_processor 能读的格式），其他工具都得先调这个 |
