...
>
> 最精确的复现锚点是第三行 —— 那是我实际读到的代码版本；前两行说明"这是从哪棵 Chromium 树里取样的"。分析对象共 5 个 TS 文件，746 行（不含 `.scss` 和 `OWNERS`）。

---

## 0. TL;DR

- Google 在 Perfetto UI 里塞了一个 **browser-side、同进程 MCP server + MCP client + Gemini 客户端**的聊天插件。架构上和 `perfetto-mcp-rs` 高度相似，核心原语都是 `query / list-tables / describe-table`，算是对我们设计的独立验证。
- 有三个细节值得直接抄进 `perfetto-mcp-rs`：**过滤 `_*` 前缀内部表**、**把 stdlib 文档 URL 塞进工具 description**、**用"请改用聚合"替换现有 row-cap 错误文案**。
- 有两个决策需要权衡：是否加**领域特化工具**（Android 进程、Macrobenchmark 切片、Chrome scroll jank 等），是否加 **UI 副作用工具**（只有和 UI 集成时才有意义）。
- 替换 Gemini：对 `com.google.PerfettoMcp` 而言是真实的移植成本（`@google/genai` SDK 和 `mcpToTool` 桥接都是强耦合）；对 `perfetto-mcp-rs` 而言则是 **non-problem** —— 我们是纯 MCP server over stdio，LLM 选择权已经下放给客户端。
...
这是 `@modelcontextprotocol/sdk` 提供的一个 **非 IPC transport**：client 和 server 共享一个内存通道，MCP 的 JSON-RPC 消息就在同一个 JS 运行时里流来流去。

为什么不直接调用工具函数？因为 `@google/genai` 的 `mcpToTool(client)` 只接受 MCP `Client` 实例，需要一个标准化的 MCP 通道来做 `listTools` / `callTool`。用 `InMemoryTransport` 既拿到了这个"标准外观"，又省掉了 stdio/HTTP 的序列化成本。

**这个 pattern 值得记一笔**：它让你以近乎零成本获得"MCP 工具声明格式"的好处（schema 统一、工具清单自动化、参数校验 via zod），同时完全不用考虑跨进程通信。对任何需要"让一个 LLM 调用一批工具"但又不想真的起一个外部进程的场景都适用。
...
- **`mode: AUTO`**：让 LLM 自主决定是否调用工具。对比 `mode: ANY`（强制调用）、`mode: NONE`（禁用）—— AUTO 是默认选项，适合这种"有时候就是闲聊"的场景。
...
**"通过文件上传来填一个 string 设置"** 是个有意思的 UX 选择（`index.ts:86-124`）：用户写一个长 system prompt 在本地文件里，上传进来，plugin 把它附加到 systemInstruction。好处是长 prompt 用文件编辑器写体验更好；坏处是没法在 UI 里直接编辑。
...
**结论**：`perfetto-mcp-rs` 现有的 `sanitize_glob_param` 是正确的工程实践，别学 Google。
...
这是一个**零参数的快捷工具** —— 它所做的事情不超过 `execute-query('SELECT * FROM process')`。存在的意义：

1. **降低 LLM 的"冷启动负担"**：不需要先 `list-interesting-tables` 再 `list-table-structure('process')` 再写 SQL，直接调一下就有结果。
2. **无参数 → zero cost**：没有参数 schema，没有参数校验错误，没有转义问题。
3. **名字是自说明的**：LLM 看到 `perfetto-list-android-processes` 会立刻知道这是用来干什么的 —— 比看到 `execute-query` 然后推理出"我该查 process 表"要省很多 token。

**但**：它叫 `list-android-processes` 却用了通用的 `process` 表，在非 Android trace（比如 Chrome、Linux、Mac）上也会工作。名字里的 "Android" 是暗示"主要对 Android 用户有用"而不是"只对 Android trace 有效"。
...
**这是全插件最精巧的工具**。它不是通用的 list-tables 或 query —— 它是一个**为 Jetpack Macrobenchmark 用户量身定做的一键入口**。description 里甚至包含了领域知识：
...
（被测 APK 的 package name 通常是 benchmark package 去掉 `.benchmark` 后缀。）

然后紧跟一句"But this is not always true, so ask the user if it's missing" —— 让 LLM 在命名约定不成立时向用户确认。

**这是"领域特化工具"的典范**：它的代价是工具表多一项、description 占 ~50 tokens，收益是把一个原本需要 LLM 反复试错的工作流（"这个 trace 是个 benchmark 吗？是在测什么 app？时间范围是多少？"）压缩成一次工具调用 + 命名约定推断。
...
LLM 调用这个工具会**在 Perfetto UI 的 SQL 查询面板里打开一个新的结果 tab**。返回值永远是 `"OK"` —— 对 LLM 来说这个工具是"纯副作用"，唯一的反馈是"成功了"或"失败了"。

用途：LLM 写了一条值得用户自己看的 SQL 时，把它 surface 出来让用户可以交互式地再跑 / 复制 / 修改。
...
LLM 发现一个可疑的长任务后，可以调一下 `show-timeline({timeSpan: {start, end}, focus: {table: 'slice', id: 12345}})`，用户立刻看到相关段被放大 + 对应 slice 被高亮选中。这相当于 Chrome DevTools AI 的 `selectEventByKey` 的翻版。

**`startTime`/`endTime` 是 string 而不是 number** —— 因为 Perfetto 时间戳是 `bigint`（纳秒），会超出 JS `number` 安全整数范围。参数 schema 用 `z.string()` 接收，服务端 `BigInt(...)` 转换。这对 LLM 友好（JSON 只有 number，string 是唯一安全选项），对 schema 实现方不友好（需要加断言）。
...
1. **5000 行上限**：和 `perfetto-mcp-rs` 一致。这个数字似乎是业界共识。
2. **错误文案**：`"Query returned too many results, max 5000 rows. Results should be aggregates rather than raw data."` —— 后半句比 `perfetto-mcp-rs` 现用的 *"Add a LIMIT clause or narrow your WHERE condition"* 更有建设性（见 §7）。
3. **`bigint → Number` 精度丢失**：Perfetto 的 `ts`（纳秒时间戳）经常超过 `2^53`，这里无脑转成 Number 会悄悄丢精度。JSON.stringify 对 bigint 原生不支持，所以不转就会抛异常；他们选择了"丢精度但能序列化"。在 LLM 分析场景里通常不致命（纳秒级别的 rounding error 不改变结论），但在需要做精确时间差的任务里会埋坑。`perfetto-mcp-rs` 的 `query.rs` 也应该面对同样的选择，值得回头看一眼。
...
**反模式观察**：Google 几乎没有用传统的长 systemInstruction 来塞领域知识，而是把领域知识切碎后塞进每个工具的 description。这样做的好处是：

- 工具描述只有在 LLM 决定考虑调用该工具时才被重点关注
- 不同工具的约束不会互相污染
- 描述随工具一起在 `tools/list` 里分发，自然分片

坏处：

- 总 prompt 体积比集中式长 prompt 更大（每个工具都要重复"this is a perfetto tool"之类的上下文）
- 升级需要改代码而不是改配置
...
- 用的是 `sendMessageStream`，响应是一个 async iterator，每个 chunk 是一个 `GenerateContentResponse`
- 每个 chunk 里的 `parts` 被分类为 thoughts / toolcalls / text 三种，分别追加进消息列表
- `toolcall` 只展示函数名（不展示参数），避免刷屏
- `updateAiResponse` 做 streaming concat —— 如果上一条消息也是 AI 的，就 append；否则新开一条

### 5.3 没有持久化

**会话状态全在内存里**。组件重建就丢 —— `constructor` 里初始化了一条欢迎语：
...
路由切换 + 重进 `/aichat` 就是新会话。这是 ChatPage 组件的事情 —— 底下的 `chat` 对象（`ai.chats.create(...)`）其实是 onTraceLoad 里 new 出来、整个插件实例持有的，但 `ChatPage` 里没有从已有 chat 恢复历史消息到 `messages[]` 数组的逻辑。结论：UI 上看到的对话历史是暂时的，但 LLM 侧的 chat session 其实仍在活着（累积着完整的 multi-turn context）。

这是个值得警惕的设计 —— 用户感知"新的对话"，但实际上 Gemini 侧仍有 context 累积。对用户信任和成本都不算透明。
...
**最低限度的错误处理**：catch + 日志 + 屏幕上显示 `error.toString()`。没有重试、没有区分网络错误和配额错误、没有超时、没有取消。对原型来说够了，对生产不够。
...
- **运行形态**决定了 `load_trace` 工具的必要性：插件被动接受已加载的 trace，我们必须暴露加载入口。
- **运行形态**决定了 UI 副作用工具的可行性：没 UI 就没有 "show timeline" 可言。
- **LLM 绑定**决定了 API key / 模型设置 / thinking 展示是不是本工具的责任。
- **工具表规模**的差异来自一个设计哲学：Google 更倾向于"为常见场景造预制工具"，我们更倾向于"保持工具表小、靠 SQL 灵活性兜底"。
...
**收益**：`list_tables` 的输出体积可以减半甚至更多。LLM 不再在 `_counter_forest`、`_slice_forest` 之类看不懂的表里游泳。
...
如果带 GLOB 参数的分支也要过滤，加同样条件即可。建议在 description 里也加一句 `"internal tables prefixed with '_' are excluded; use execute_sql directly if you need them"`，以防有高级用户真的想看。
...
**代价**：description 从 ~200 字符涨到 ~800+ 字符，每次 `tools/list` 都多几百 token。
**收益**：给 LLM 一个明确的"有疑问就去看这里"的指向。如果客户端有 WebFetch 能力（Claude Code、Claude Desktop 带 fetch 插件、自建客户端），LLM 会真的去拿文档。
...
| 候选 | 内部 SQL | 价值 |
|---|---|---|
| `list_processes` | `SELECT pid, name, start_ts, end_ts FROM process` | Android/Linux trace 分析的起手式 |
...
**建议态度**：先加 `list_processes`（低风险、明显好处），观察 Claude Code 会话里 LLM 的行为变化。如果确实减少了试错，再考虑加其他的。**避免一上来就加 4 个**——工具表膨胀会让每次 `tools/list` 响应变大，且 LLM 选工具的难度也会上升。
...
在 MCP 架构里，这个"用户确认"本来就是**客户端的 permission mode** 负责的 —— Claude Code 默认就会在每次工具调用前显示参数 + 等待用户确认。我们**不需要**也**不应该**在 description 里要求 LLM "show SQL first" —— 那等于把客户端层的职责搅进服务端。

**但**：对于裸用的客户端（无 permission mode），这是个差异化场景。可以考虑在 description 里加一句可选建议 *"For destructive-looking queries (DROP, DELETE, CREATE), consider confirming with the user first"* —— 虽然 trace_processor 实际上是只读的，DROP 不会真生效，但 LLM 的不确定性本身值得一个 guard。
...
2. **在 `perfetto-mcp-rs` 里**谈"换模型"（实际上这是伪命题）

先说结论：**问题 2 不存在**。`perfetto-mcp-rs` 是纯 MCP server over stdio —— 它不 own 任何 LLM，LLM 选择完全由连接上来的客户端决定。Claude Code 用 Claude 系列，Claude Desktop 也是 Claude，Cody 用 Anthropic / OpenAI，ChatGPT Desktop 用 GPT 系列 —— 它们各自把 `perfetto-mcp-rs` 的工具挂到各自的 function calling 上。我们不需要做任何事情来"支持其他模型"，换客户端就是换模型。
...
大同小异。都有 function calling，都有 JS/TS SDK，都需要自己写 agentic loop。优先级低，除非有特殊合规需求（Mistral 是欧洲数据主权卖点）。
...
- 多一跳，延迟 +50~200ms
- OpenRouter 收 5% 佣金
- 非所有模型都支持 tool calling；选模型需要看 OpenRouter 的 function calling 支持列表
...
**现实评估**：在 Perfetto 分析这种需要 multi-step tool use 的场景，**本地模型目前还不太够用**。经验上，只有 ≥70B 级别的 Llama / Qwen / Mistral 能 reliably 在 2-3 跳之内完成"list_tables → table_structure → execute_sql"的流程，且错误率明显高于 Gemini 2.5 Pro / Claude Opus / GPT-4o。如果目标用户是开发者自己，这个代价可以接受；如果目标是 non-technical end-user，不推荐。

**特殊考虑**：浏览器插件场景下本地模型有个 UX 优势 —— 数据完全不出本地，trace 里的敏感信息（设备 ID、用户数据、源码 URL）都不外发。对合规敏感的团队是刚需。
...
回到本项目。我们需不需要做任何"换模型"的准备工作？**几乎不需要**。但有几个点值得想清楚：

1. **我们隐式依赖了 MCP 客户端的 agentic 能力**。Claude Code 会自动做 multi-turn tool use；小客户端不会。我们的错误消息 nudge（"call list_tables then table_structure"）隐含假设 LLM 会自己循环 —— 对非 agentic 客户端无效。这不是缺陷，是 MCP 生态的本质 —— 但值得在 README 里标注 "works best with agentic MCP clients"。
...
最后值得记录的一条：**把领域知识切碎后塞进每个工具的 description，而不是集中在 systemInstruction 里**（§4.4），是 `com.google.PerfettoMcp` 最可复制的 prompt engineering 决策。在 MCP 架构下这个决策尤其合适 —— 因为 description 会被 `tools/list` 自然分发到任何连接的客户端，而 systemInstruction 是客户端自己的事。
